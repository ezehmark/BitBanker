...
const sendMsg = async () => {
	setLoading1(true);
          await axios.post("https://mybackend-oftz.onrender.com/CSAgent",[{name:"John Doe",msg:chat}])
          .then((response)=>setChecker(response.data.feedback)).catch((error)=>setChacker(error.message)).finally(()=>{setChat("");console.log("All is done");setLoading1(false)})}
  ...
  <g stroke="#00ffcc22" stroke-width="0.5">
    ...
    <path d="M0,300 Q400,500 800,300" fill="none"/>
      ...
      <div className="titleAndLogo">
        ...
        <div style={{color:"white"}}className="app-name">CS Client</div>
      </div>
	  ...
	  <button
	    ...
	    >
              <div className="buttonTitle">{loading1?"Sending":"Send now"}</div>
            </div>                                                                                </button>
